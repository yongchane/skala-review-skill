...
ChatGPT·Gemini·Claude 웹에서는 아래 두 줄만 복사해 보내세요.
...
정상적으로 시작되면 AI가 `오늘 무엇을 배웠나요?`라고 물어봅니다. 사용자는 생각나는 키워드나 정리하다 만 메모를 보내면 됩니다. 별도의 설치 명령이나 기술 용어를 입력할 필요가 없습니다.
...
터미널을 사용할 수 없다면 위의 **웹 실행 번들 링크와 시작 문구**를 사용합니다. 웹 AI가 GitHub를 읽지 못하는 대화에서는 [웹 실행 번들](platforms/web/SKALA-REVIEW-WEB-BUNDLE.md)의 **파일 전체를 복사해 붙여 넣거나 `.md` 파일로 첨부**합니다.

## 환경 자동 판별이 필요한 경우

아래 README 직접 링크 방식은 하나의 요청으로 웹·Codex·Claude Code 환경을 판별해야 할 때 사용하는 고급 방법입니다. 일반 웹 사용자는 앞의 `웹에서 가장 쉽게 시작하기`만 따르면 됩니다.

```text
https://github.com/yongchane/skala-review-skill/blob/main/README.md
```

다음 문장을 그대로 복사해 요청합니다.
...
README는 웹 AI를 자기완결형 웹 번들로, Codex와 Claude Code를 각 설치 경로로 안내합니다. 이 방식은 ChatGPT 웹에서 `README 열기 → 라우터 판별 → 첫 복습 질문`까지 확인했습니다. 단, 웹 AI의 GitHub 탐색 권한은 서비스·계정·대화마다 다르므로 README를 읽지 못하는 경우에는 웹 번들 파일을 직접 첨부해야 합니다.
...
웹에서는 [웹 실행 번들](platforms/web/SKALA-REVIEW-WEB-BUNDLE.md) 직접 링크와 `SKALA 복습 시작해줘`를 사용하는 것이 가장 간단합니다. README 직접 링크를 사용한 경우에는 최상단 라우터가 웹 실행 번들로 연결합니다. 두 방식 모두 설치 불가 안내나 코드 리뷰 설명을 하지 않고 첫 복습 질문부터 시작하는 것이 정상 동작입니다.
...
가장 간단한 방법은 웹 실행 번들 링크를 직접 전달하는 것입니다.
...
웹 AI가 번들을 읽으면 로컬 설치 없이 현재 대화에서 복습 하네스를 따릅니다. 저장소 구조나 README 라우터를 이해할 필요가 없습니다. GitHub 접근이 차단된 경우에는 아래 대체 방법을 사용합니다.
