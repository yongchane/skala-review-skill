...
수업 내용을 완벽하게 정리하지 못해도 괜찮습니다. 생각나는 키워드나 직접 작성한 메모만 보내면 AI가 질문으로 복습을 돕고, 그 내용을 짧은 복습 글이나 실습 보고서 작성 가이드로 정리해 줍니다.
...
2. **복습 글·보고서 작성 도우미**
   Velog·Notion·Tistory용 짧은 복습 글 또는 실습 보고서의 목차·작성 방법·문장 초안을 만듭니다.
...
2. 목적에 맞는 프롬프트를 선택합니다.
   - [질문 복습 프롬프트](web-prompts/skala-review-prompt.md): 질문으로 복습한 뒤 글·보고서 작성으로 이어가기
   - [빠른 정리·작성 프롬프트](web-prompts/skala-quick-draft-prompt.md): 질문 없이 빠르게 요약하거나 글·보고서 구조 만들기
...
- 질문 복습, 복습 글·보고서 작성, 지난 학습 재복습, 빠른 요약 기능
...
- 추천 보고서 목차
- 각 항목에 들어갈 내용과 작성 근거
- 사용자가 제공한 결과를 반영한 짧은 문장 초안
- 표·그래프·코드를 배치할 위치
- 추가로 필요한 데이터 또는 검증 항목

제공되지 않은 수치·출처·분석 결과는 만들지 않고 `[실행 결과 입력]`, `[근거 확인 필요]`로 표시합니다.
...
이 프로젝트는 [MIT 라이선스](LICENSE)로 제공됩니다. 비전공자를 위한 쉬운 설명, 복습 흐름, 웹 프롬프트, 글·보고서 출력 형식, 설치와 검증 개선에 누구나 기여할 수 있습니다.
