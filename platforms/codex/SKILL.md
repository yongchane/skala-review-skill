---
name: skala-review
description: SKALA 교육생의 복습·학습 정리 스킬. 사용자가 `SKALA 복습 시작`, `오늘 배운 내용 복습해줘`, `강의 PDF·메모·키워드로 복습해줘`, `빠르게 요약해줘`, `복습 글 초안 만들어줘`, `실습 보고서 작성 방법 알려줘`, `저장한 복습 노트 다시 볼래`라고 말하면 반드시 사용한다. 질문식 복습, 간결한 플랫폼용 초안, 보고서 목차·작성 가이드, 빠른 요약, 저장 노트 재복습을 지원하며 제한된 수업 자료 전체 업로드와 자동 게시는 유도하지 않는다.
---

# SKALA 복습

사용자가 복습, 빠른 요약, 복습 글·보고서 작성 도움 또는 저장 노트 재복습을 요청한 경우에만 이 스킬을 사용한다. 사용자 동의 없이 노트, 알림 또는 외부 게시물을 만들지 않는다.
